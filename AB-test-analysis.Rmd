...
calcIntervalEstimate <- function(variant, variantCnt) {
...
We have gathered some data on both variants now lets compare these insights. First lets visualise the proportion of resolved to unresolved queries the difference of resolved queries between variants
...
```
As we can see from stacked bar chart there is a higher proportion of resolved queries on the new variant over the old. Additionally there is a higher proportion of visits below 180 seconds on the new site.


Histogram of Time on VariantA

As we can see from the distribution of times across the histogram below our times for each visit fall between ~200 and ~400. This confirms our finding that the probability of a visit being below 180 seconds (3 minutes) is 0.0. We can further confirm this by looking at our minimum time found in the variant a sample.
...
As we can see the distribution of times differs for variant B across the histogram. Our time distribution range is significantly lower on the min and max. Additionally we see that there is indeed records below the 180 second thus confirming our finding that the probability of a visit being below 180 seconds (3 minutes) is 0.327. We can further confirm this by looking at our minimum, average and maximum times found in the variant a sample.

```{r}
sprintf("Variant B minimum time: %s, average time: %s, maximum time: %s", min(variantB$Time),  mean(variantB$Time),max(variantB$Time))
```
As we can see the minimum is as low as ~93 seconds, over 100 seconds faster than our previous minimum and our max time is 10% higher than our previous average at ~319 seconds but ~80 seconds lower than our previous max. The average time on the new variant is quite close to our 180 second target at 207 seconds further reinforcing the probability found for this metric.
...
```
As we can see from our result the sample mean is higher at 207.161 than the null hypothesis value of 180.

There is convincing evidence as the p-value is very small that the mean is not consistent with the null hypothesis value thus we can reject the null hypothesis.
...
```
We see here our mean is much further away from our target of 180 seconds, we can reject the null hypothesis given our p-value is even smaller than the previous test.
...
Variant B
```{r}
calcIntervalEstimate(variantB, variantBCnt)

```
Variant A
```{r}
calcIntervalEstimate(variantA, variantACnt)
```
### Conclusion
...
2. A higher resolved queries proportion.
